/**
 * Material and texture control tools for CC4.
 */
...
export function registerMaterialTools(server: McpServer, bridge: CC4Bridge) {
  server.tool(
    "get_material_info",
    ...
    async ({ avatar_name }) => bridgeCall(
      ...
      (info) => {
        ...
        if (meshNames.length === 0) {
          return "No meshes found on the avatar. The avatar may not have material data, or GetMeshNames() may not be available in this CC4 version.";
        }
