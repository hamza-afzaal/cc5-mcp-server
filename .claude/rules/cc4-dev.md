...
- Dev copy: when CC4 is started with `CC4_DEV_MODE=1` and `CC4_PLUGIN_DEV_DIR=<repo>/cc4-plugin`, main.py imports server/cc4_api from the repo, so edits there are what `/reload` loads (no reinstall).
